...
class AuthenticationTest < ActionDispatch::IntegrationTest
  ...
  def test_authentication_proxy_success
    ...
    scalarm_user = mock 'scalarm user' do
      stubs(:id).returns(user_id)
      stubs(:convert_to).with(Scalarm::ServiceCore::ScalarmUser).returns(em_scalarm_user)
    end
  ...
  def test_basic_auth_fail
    ...
    get '/', {}, {
               'HTTP_ACCEPT' => 'application/json',
               'HTTP_AUTHORIZATION' =>
                   ActionController::HttpAuthentication::Basic.encode_credentials(login, wrong_password)
           }
